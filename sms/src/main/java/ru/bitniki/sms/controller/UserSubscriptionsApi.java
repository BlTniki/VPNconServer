...
public interface UserSubscriptionsApi {
    ...
    @Operation(summary = "Removes subscription from user", description = "", tags = { "user_subscriptions" })
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "The removed user subscription", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UserSubscriptionResponse.class))),

            @ApiResponse(responseCode = "404", description = "User or subscription not found", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class))),

            @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class))) })
    @RequestMapping(value = "/user_subscriptions",
            produces = { "application/json" },
            method = RequestMethod.DELETE)
    Mono<ResponseEntity<UserSubscriptionResponse>> userSubscriptionsDelete(@NotNull @Parameter(in = ParameterIn.QUERY, description = "user telegram id", required = true, schema = @Schema()) @Valid @RequestParam(value = "user_id", required = true) Long userId
    );
}
